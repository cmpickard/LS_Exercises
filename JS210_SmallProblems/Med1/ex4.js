...
// with a stack of [3, 6, 4] (where 4 is the topmost item in the stack) and
// a register value of 7, the MULT operation mutates the stack to [3, 6] (the
//   4 is removed), and the result of the multiplication, 28, is left in the
//   register. If we do another MULT at this point, the stack is mutated to
...
function minilang(strCommands) {
  let register = 0;
  let stack = [];
  let commands = strCommands.split(' ');
  let error = false;
...
// In particular, the function should detect and report empty stack
// conditions, and invalid tokens in the program. Ideally, the function
// should return an error message if an error occurs, or undefined if
// the program runs successfully.
console.log(minilang('POP POP'));
console.log(minilang('PLOP PLOP'));