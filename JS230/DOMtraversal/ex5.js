...
// generation. You can assume that only non-negative integers will be
// provided as arguments. Following are some sample output to help you
// test your code:
...
function colorGeneration(genNum) {
  let gens = [[document.body]];
  let count = genNum;

  while (count > 0) {
    let prevGen = gens.slice(-1)[0];
    let nextGen = [];
    prevGen.forEach(node => {
      [...node.children].forEach(child => nextGen.push(child));
    });
  ...
  gens.pop().forEach(node => node.classList.add('generation-color'));
}
