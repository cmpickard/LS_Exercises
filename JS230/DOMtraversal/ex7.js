...
// nested arrays. Each element in the DOM is represented as
...
// represented as ["PARENT_TAG_NAME", []]. For instance, if the HTML
// doesn't have any elements inside the body, the result array would be:
// ["BODY", []]. Likewise, if the HTML only has a div element as its content,
// the result array would be: ["BODY", [["DIV", []]]].

// Go over the examples below to better visualize how the DOM is
...
function nodesToArr() {
  function traverse(root) {
    return [root.nodeName, [...root.children].map(child => traverse(child))];
  }
