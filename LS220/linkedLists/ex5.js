// Given the head of a singly linked list containing integers, create a new
// linked list where each node represents the frequency of a unique element
// from the original list. The order of nodes in the new list doesn't matter
// Return the head of this new frequency list.
...
function createFrequencyList(head) {
  ...
  while (curr) {
    count.set(curr.val, (count.get(curr.val) || 0) + 1);
    curr = curr.next;
  }

  let dummy = new ListNode();
  let currFreqList = dummy;

  count.values().forEach(value => {
    currFreqList.next = new ListNode(value);
