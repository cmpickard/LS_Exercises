...
#     even be another collection - but it is nevertheless just one value. Thus, 
#     #map returns an Array that has the same number of elements that the 
...
def map(collection)
  collection.each_with_object([]) { |el, obj| obj << yield(el) }
end
